...
Vercelでは、リポジトリをインポートして環境変数 `ASANA_ACCESS_TOKEN` と `OPENAI_API_KEY` を設定すると、同じ公開URLでアプリ、Asana API、OpenAI APIを使えます。Vercel KV/Upstashを追加すると、プロジェクト状態をクラウドDBへ保存できます。
...
アプリの状態は標準ではブラウザのlocalStorageに保存されます。Vercel KV/Upstash互換のRedisを設定すると、`クラウドDB` パネルから保存・読込できます。
...
4. クラウド保存を使う場合は、Vercel KV/Upstashを追加して `KV_REST_API_URL` と `KV_REST_API_TOKEN` を追加します。
5. Deployします。
6. アプリ上で `API接続確認` を押します。
